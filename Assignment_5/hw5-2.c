...
int main (int argc, char** argv) {
    ...
    while(1) {
        ...
        */
        ...
        //printf("cwd=%s, home=%s, pos=%d, prompt=%s logname=%s", cwd, getenv("HOME"), pos, &cwd[pos],loginName);
        if(pos>=homeLen) {
            cwd[pos-1]='~';
            showPath=&cwd[pos-1];
        }
        ...
        */
        ...
        if (strcmp(exeName, "cd") == 0) {
            if (strcmp(argVect[1], "~")==0){
                ...
                chdir(argVect[1]);
                char *arg[]={"cd",argVect[1],(char *)0};
            }
            continue;
        }
        if(strcmp(exeName,"ls")==0){
            if(idx==2){
                char *arg[]={"ls",(char *)0};
            }
            else if(idx==3){
                char *arg[]={"ls",argVect[1],(char *)0};
            }
            else if(idx==4){
                char *arg[]={"ls",argVect[1],argVect[2],(char *)0};
            }
            else if(idx==5){
                char *arg[]={"ls",argVect[1],argVect[2],argVect[3],(char *)0};
            }
        }
        pid = fork();
